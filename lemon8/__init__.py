...
class User:
    ...
    def get_profile_page(self, use_cached=True):
        ...
        """
        if not use_cached:
            return self.get_forced().content.decode('utf-8').split('\n')
        return self.get().content.decode('utf-8').split('\n')
...
if __name__ == '__main__':
    pprint.pprint(Lemon8().user('chancejeastkay').get_details(), indent=1)
